...
- **Project, don't dump.** Returning whole documents is the token-equivalent of `SELECT *` — mongosh prints the full BSON and it is re-read into context on every later tool turn. Pass a projection to `find` (`{_id: 0, field: 1, …}`) and shape `aggregate` with a final `$project` to only the fields you need. For agent consumption, emit `EJSON.stringify(result, null, 0)` (one line, no `ObjectId(...)`/`ISODate(...)` wrappers) rather than the default pretty BSON. `--quiet` (already standard here) strips only the banner, not the document bodies.
