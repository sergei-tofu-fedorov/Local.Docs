...
| `amplitude_us` (Amplitude events bridge, iOS 213333 + web 586241 + FS-iOS 760259, rolling 90d) | BigQuery | prod=`inv-project` · stage=`invoicesapp-project-test` | `Tofu.AI.Backend` (amplitude-export tick) | `Analyses:Amplitude` + `BigQuery:AmplitudeDatasetId` | [query guide](bigquery-agent-guide.md) |
...
| `stripe_us` (web-subscription Stripe mirror: customers, transactions, connected accounts) | BigQuery | prod=`inv-project` | `Tofu.AI.Backend` (stripe-ingest tick) | `Analyses:Stripe` + `BigQuery:StripeDatasetId` | [query guide](bigquery-agent-guide.md) |
| `meta_us` (Meta Marketing API mirror: campaigns/adsets/ads/creatives/insights + status history + a `vw_`/`tvf_` reporting layer for landing-page and creative reports; two ad accounts — `act_2725877157763325` FSM, `act_318657674434347` Invoice Maker) | BigQuery | prod=`inv-project` | `Tofu.AI.Backend` (four meta-ingest ticks) | `Analyses:Meta` + `BigQuery:MetaDatasetId` | [query guide](bigquery-agent-guide.md) |
