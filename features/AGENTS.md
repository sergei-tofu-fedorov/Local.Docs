...
| `FS-1444` | Префилл авторизованного email при создании Stripe Connect connected-аккаунта (только BFF `Invoices.Backend`): берём `AuthenticationInfo.Email` из контекста запроса (без доп. запроса в Tofu.Auth), протаскиваем в `PreAuth`, ставим `Email` + `Individual.Email`; лечит login-wall C2/M1 из `stripe-onboarding-redesign`. **Доки в репозитории `Invoices.Backend`** → `Docs/features/FS-1444/`. | в `Invoices.Backend` |
