...
| C. Read from Playfair DWH (BigQuery `data_layer.mongo_invoices_*`) | Re-enable the dormant Playfair Mongo streams; aggregate in BQ; `Tofu.AI.Api` reads BQ instead of Mongo. | Rejected for v1. Daily staleness, PII-hashing on the invoice loader strips item text, cross-team coupling. |
...
See per-option detail below. Cross-reference: Playfair DWH inventory in [`dwh.md`](dwh.md); Amplitude event production from the BFF is referenced in [`../analyses/fsm-fit/analytics-events.md`](../analyses/fsm-fit/analytics-events.md) (write side) but not as an analysis input today.
...
## Option C — Read from Playfair DWH (BigQuery)

**Shape.** Re-enable the dormant Playfair Mongo streams listed in [`dwh.md`](dwh.md) (the `invoices`, `estimates`, `clients`, `accounts` loaders are schema-defined but commented out at `mongo_source.py:354-364`). Aggregate in BQ via dbt models. `Tofu.AI.Api` then reads finished metrics from BQ instead of Mongo.
...
- **Joinable with existing Playfair data** — LTV cohorts, attributed revenue, Amplitude experiments are all already in BQ keyed by `account_id ↔ user_id` ([`dwh.md`](dwh.md) § Downstream consumers). Could enable cross-cohort analyses cheaply.

**Cons.**

- **Daily staleness.** Playfair runs at 00:05 PT / 03:05 UTC — cold-start latency moves from ~2h to ~24h. Below the proposal-surface SLA.
- **PII hashing on the invoice loader.** `MongoInvoicesStream` MD5-hashes Client struct values on write ([`dwh.md`](dwh.md) § Streams). Doesn't affect `account_metrics` directly (we don't store Client fields there), but it shows the loader's PII regime is owned by Playfair and not under our control — a future metric needing raw client data would force re-coordination.
- **Cross-team coupling.** Why the streams were disabled is not documented (cost? PII concerns? broken?). Re-enabling them is a Playfair-owner conversation, not a `Tofu.AI.Backend` decision. See [`dwh.md`](dwh.md) § Open questions.
...
**Shape.** Use the existing Amplitude pipeline (already wired into Playfair as `amplitude_users_in_experiments_clean`, referenced in [`dwh.md`](dwh.md) § Downstream consumers) as a signal source.
...
| Concern | A. Direct Mongo+PG | B. Event-sourced | C. Playfair DWH | D. Amplitude |
|---|---|---|---|---|
...
| New cross-repo contracts | 0 | event schema × N services | re-enable Playfair streams | none net-new (Amplitude already prod) |
| Producer-side read load | Mongo secondaries, PG | event broker | none on Mongo | none on Mongo |
| PII control | local | depends on event schema | hashing owned by Playfair | event-set-determined |
...
3. Whether the Playfair team has a use case to re-enable the dormant Mongo streams independent of WEB-1523 — if so, Option C becomes free for us and worth re-evaluating for the v2 metrics that don't need sub-day freshness.
...
- [ ] **Who would own the Playfair stream re-enablement conversation?** Tied to [`dwh.md`](dwh.md) § Open questions — needs a name before Option C can be costed seriously.
