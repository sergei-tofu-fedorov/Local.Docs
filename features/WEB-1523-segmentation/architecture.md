...
The two layers are independent — a failure in one (e.g. OpenAI outage) doesn't block the other (metrics keep refreshing). Per-analysis paths in Layer B are also independent of each other: a churn-risk bug doesn't block FSM-fit. The **FSM-fit audience filter lives in Layer B** (the account-maturity gate over the source `accounts` collection), not Layer A — `account_metrics` is analysis-agnostic. The earlier FSM-using-account exclusion (a `jobs.Jobs` PG probe) was removed — job filtering is not used at this stage. See [`implementation/analyze.md`](implementation/analyze.md) § Audience eligibility.
