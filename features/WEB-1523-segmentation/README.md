...
**Still NOT built:**
- **Presidio redaction** (analyze stage) — implemented on the `feature/WEB-1555-presidio` branch (not yet on the merged baseline); the merged WEB-1555 payload still goes fully unredacted. Per the 2026-05 A/B ([`investigation/privacy.md`](investigation/privacy.md) § 2a) the wired-in redaction is **notes-only** — item names are sent raw (they are the classifier's primary signal; redacting them was the dominant cause of mis-scoring).
...
| [`analyses/flexible-metrics.md`](analyses/flexible-metrics.md) | **Theory** — evolving the `account_metrics` feature store: metric-as-code catalog, additive-only schema + proto lockstep, definition-hash versioning, TTL-loop-as-backfill; **+ fast-churn topologies** (per-domain marts, BQ MVs, dbt compute-on-read over mirrors, medallion hybrid). | framework | ⬜ (design only) |
| [`analyses/metrics-gold-migration.md`](analyses/metrics-gold-migration.md) | **Theory** — phased plan to migrate `account_metrics` from the CDC-collector table to the **gold/medallion** model: expand-contract read seam, bronze/silver/gold buildout, parallel-run + parity gate, view-repoint cutover, rollback. | framework | ⬜ (design only) |
...
- ✅ **Done (WEB-1555 — analyze stage):** `account_fsm_fit` table (`V002`) + `v_fsm_fit` view (`V003`); OpenAI `gpt-4.1-nano` LLM client per [`investigation/provider.md`](investigation/provider.md); `FsmFitScorer` deterministic rule + tiering; `AnalyzeFsmFitJob` with FSM-using eligibility trim + input-hash cache + CDC-UPSERT. **Gap (merged baseline):** payload unredacted. Presidio lives on `feature/WEB-1555-presidio` and redacts **notes only** (`FsmFitPayloadBuilder.RedactNotesAsync`); item names sent raw — A/B-justified ([`investigation/privacy.md`](investigation/privacy.md) § 2a).
...
- ⬜ **Pending:** Presidio redaction code; **FSM-fit account-age audience filter** (only accounts created more than 90 days ago are *scored* — applied in `AnalyzeFsmFitJob` alongside the FSM-using exclusion, see [`implementation/analyze.md`](implementation/analyze.md) § Audience eligibility; **not** a metrics gate — `account_metrics` rows still exist for younger accounts); BFF in-app proposal surface in `Invoices.Backend`; Looker Studio dashboard + checked-in `queries/*.sql`; privacy-policy + sub-processor list update; SQL-rule baseline comparison.
